...
def build_clip_cues(segments: list[dict], clip_start: float, clip_end: float,
    ...
    if relisten:
        ...
        talk_cues = _talk_cues_from_words(anchor_cues, segments, song_spans,
                                          clip_start, clip_end, origin,
                                          llm=llm,
                                          require_llm_segmentation=force_llm_retranslate)
...
def _talk_cues_from_words(anchor_words: list[Cue], segments: list[dict],
                          ...
                          origin: float, gap: float = 0.7, max_dur: float = 6.0,
                          max_chars: int = 24, llm=None,
                          require_llm_segmentation: bool = False) -> list[Cue]:
    ...
    if llm is not None:
        try:
            ...
            if require_llm_segmentation:
                raise RuntimeError("LLM 未返回有效断句")
        except Exception as e:  # noqa: BLE001
            if require_llm_segmentation:
                raise RuntimeError(f"LLM断句失败：{e}") from e
            print(f"  [warn] LLM断句失败，退回停顿断句：{e}")
...
- 一条字幕尽量 6-22 个日文字符，最长不要超过 30 个字符；很短的“うん/はい/えー”等可单独成句。
...
def _llm_segment_ranges_chunked(
    ...
    *,
    ...
    for first, last in _chunk_word_ranges(words):
        ...
        if not ranges and len(local) > _LLM_SEGMENT_MIN_CHUNK_WORDS:
            mid = first + len(local) // 2 - 1
            print(f"  [warn] LLM断句分块 {offset + first}-{offset + last} 失败，继续拆半")
            left = _llm_segment_ranges_chunked(words[first:mid + 1], llm, offset=offset + first)
            right = _llm_segment_ranges_chunked(words[mid + 1:last + 1], llm, offset=offset + mid + 1)
            if left and right:
                out.extend((a + first, b + first) for a, b in left)
                out.extend((a + mid + 1, b + mid + 1) for a, b in right)
                continue
        if not ranges:
            return []
