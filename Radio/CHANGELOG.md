...
### Fixed — 日语 ASR 幻听 + 听不全（STT 链路四项治理，2026-06-10）

诊断（用 5 期歌枠/谈话真实产物量化）：幻听集中在音乐/BGM 段（开播等待画面 4.5 分钟连环
「ご視聴ありがとうございました」、歌词混入、全局 prompt 异节目人名被「听」出来 12 处/期），
听不全来自 120s 固定硬切截断句子（26-29 段/期恰好断在切点）与 Whisper 解码窗被幻听吃掉后
整窗跳过。**不是 prompt 模板本身的问题**，但全局 prompt 的人名会在音乐段被幻听成内容。

- `segmenter.py`｜**静音对齐切片**：silencedetect 找静音中点下刀（±40% 段长窗口，无静音回退硬切），
  `segment_seconds` 120→300。句子截断 26→2 段（ep0609 实测）。
- `stt.py`｜**三重幻听过滤**（原先 verbose_json 的置信信号完全未用）：
  ① `no_speech_prob>0.6 且 avg_logprob<-1.0`（OpenAI 同款静音判据）；
  ② `compression_ratio>2.4`（段内复读循环）；
  ③ 已知幻听短语黑名单（整段去标点后只剩黑名单短语重复、且置信不高才丢——主播真说感谢时保留）。
  纯幻听行 17→1 / 10→0（两期实测）。阈值见 `STTConfig`，config.yaml 可覆盖。
- `stt.py`｜**跨段复读坍缩**（连续 ≥3 段同文本保首段并时间），首轮与重听回填后各跑一遍。
  最长复读 9→2。
- `stt.py`｜**prompt 回声加固**：整段（去标点）是 prompt 连续子串且横跨 ≥2 词条 → 回声丢弃
  （单词条不算，主播可能真说节目名）。配合 clip 节目方案新增 `processing.stt_prompt`
  （`kb_ingest` 按方案替换全局 prompt），异节目人名泄漏 12→0。
- `stt.py`｜**空洞重听（听不全兜底）**：首轮转写后两类空洞——时间空洞（相邻段间 ≥25s 空白）
  与「窗口吞噬段」（≈30s 解码窗只有几个字，如「BanG Dream」盖住 30s 谈话）——单独切片重转，
  吞噬段仅在重听更充实时替换。ep0609 实测找回 7 处吞噬段 + 3 处空洞（含 803-833s 整段天气谈话，
  9→133 字符）；谈话区总字符量 8365→8422，反超旧管线。
- Verified：两期歌枠全量 A/B（旧产物 vs 新管线）+ 谈话区双向找回率对称（79%/79%，
  差异为两次 ASR 措辞漂移而非内容丢失）+ 多窗口逐句抽查；过滤单元断言
  （高置信「ご視聴…」保留、短文本「もう」不误杀、静音/复读/回声各判据）全过。
